...
public class Customer : MonoBehaviour
{
    ...
    private bool CanGettedHamburger = false;
    ...
    public void SetGetHamburger()
    {
        CanGettedHamburger = true;
    }


    #region["최대로 받을 수 있는 음식 개수 지정하기"] 
    public void SetMaxWantedFoodNum(int _foodnum)
    {
        max_foodnum = _foodnum;
    }
    #endregion
    ...
    private void SetWantedFoodList()
    {
        ...
        if (max_foodnum > 1)
        {
            foodnum = Random.Range(1, max_foodnum + 1);
        }
        ...
        for (int i = 0; i < foodnum; ++i)
        {
            ...
            /*
            if (foodname.Equals(orderinfo.GetFoodNameByTypeandNumber(typeof(BurgerInfo.Hamburger), 0))) 
            {
                foodname = orderinfo.GetFoodNameByTypeandNumber(typeof(FriesInfo.Fries), 0); 
            }
            */
    ...
    private IEnumerator GetFoodCoroutine()
    {
        ...
        while (IstimeOver == false)
        {
            if (food_getted != null)
            {
                ...
                {
                    PhotonNetwork.Destroy(food_getted);
                }
                else
                {
